...
resize = T.Compose([T.ToPILImage(), T.Resize(40, interpolation=Image.CUBIC), T.ToTensor()]) # 40x40 image
...
def get_screen():
    screen = env.render(mode='rgb_array', highlight=False).transpose((2, 0, 1))
    screen = np.ascontiguousarray(screen, dtype=np.float32) / 255
    screen = torch.from_numpy(screen)
    #plt.figure()
    #plt.imshow(screen.cpu().permute(1, 2, 0).numpy(),interpolation='none')
    #plt.imshow(resize(screen).unsqueeze(0).to(device).cpu().squeeze(0).permute(1, 2, 0).numpy(),interpolation='none')
    #plt.show()
    return resize(screen).unsqueeze(0).to(device) # 40x40 image
    #return screen.unsqueeze(0).to(device)   # 364x384 image side
...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        for i in range(env.n_agents):
            ...
            obs, reward, done, agents, info = env.step(action.tolist(), i)
